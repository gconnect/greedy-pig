...
async function handle_advance(data) {
  ...
  try {
      ...
      if (JSONpayload.data == '' || null) {
        ...
        await reportHandler('Result cannot be empty')
        return 'reject'
      }
...
async function handle_inspect(data) {
  ...
  if (JSONpayload.data == '' || null) {
   ...
   if (JSONpayload.method === 'balance') {
      try {
        console.log('getting balance ')
        
        // let notice = wallet.ether_withdraw(etherPortalAddress, JSONpayload.address)
        let amount = wallet.balance_get(JSONpayload.account).ether_get()
        // console.log('notice ', notice)
        console.log('bal ', amount)
       
      } catch (e) {
        await reportHandler(e)
        return 'reject'
      }
